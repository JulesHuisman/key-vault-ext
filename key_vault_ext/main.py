...
from key_vault_ext.extension import KeyVaultExtension

APP_NAME = "KeyVault"

log = structlog.get_logger(APP_NAME)

ext = KeyVaultExtension()
...
def initialize(
    ...
    """Initialize the KeyVault plugin."""
    try:
...
@app.command()
def hydrate() -> None:
    """Hydrate a .env file with Key Vault Secrets."""
    try:
        ext.hydrate()
    except Exception:
        log.exception("describe failed with uncaught exception, please report to maintainer")
...
def main(
    ...
    """Simple Meltano extension that wraps the az keyvault CLI."""
